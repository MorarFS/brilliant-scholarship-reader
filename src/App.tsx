...
const ACTIVITY_STORAGE_KEY = "brilliant-paper-activity-v1";
...
function loadActivity(): Record<string, string> {
  try {
    const value = JSON.parse(localStorage.getItem(ACTIVITY_STORAGE_KEY) || "{}") as unknown;
    if (!value || typeof value !== "object" || Array.isArray(value)) return {};
    return Object.fromEntries(Object.entries(value).filter((entry): entry is [string, string] => typeof entry[1] === "string" && !Number.isNaN(Date.parse(entry[1]))));
  } catch { return {}; }
}
...
export default function App() {
  ...
  const [activity, setActivity] = useState<Record<string, string>>(loadActivity);
  ...
  useEffect(() => { try { localStorage.setItem(ACTIVITY_STORAGE_KEY, JSON.stringify(activity)); } catch { /* Activity ordering remains active for this visit. */ } }, [activity]);
  useEffect(() => {
    let cancelled = false;
  ...
  const sourcePapers = useMemo(() => {
    if (view === "saved") return [...savedPapers].sort((a, b) => (activity[stablePaperId(b)] || "").localeCompare(activity[stablePaperId(a)] || ""));
  ...
  }, [activity, data, savedPapers, view]);
  ...
  const filtered = useMemo(() => {
    ...
    }).sort((a, b) => { if (view === "saved") return (activity[stablePaperId(b)] || "").localeCompare(activity[stablePaperId(a)] || ""); const yearOrder = b.publicationDate.slice(0, 4).localeCompare(a.publicationDate.slice(0, 4)); if (yearOrder) return yearOrder; return sort === "score" ? b.relevance.score - a.relevance.score : b.publicationDate.localeCompare(a.publicationDate); });
  }, [activity, sourcePapers, query, journal, focus, year, sort, view]);
