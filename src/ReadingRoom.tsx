...
export default function ReadingRoom({ paper, annotations, onAddAnnotation, onUpdateAnnotation, onDeleteAnnotation, onEnsureSaved, onClose }: Props) {
  ...
  return <div className="reading-room" role="dialog" aria-modal="true" aria-labelledby="reading-room-title">
    ...
    <div className="reading-room__workspace"><main className="reader-document" aria-label="Paper content">
      ...
      {readerState === "ready" && mode === "text" && <><div className="reader-selection-tools"><p>Select a passage below, then highlight or hear it.</p><div><button type="button" onClick={captureSelection}>Highlight selection</button><button type="button" onClick={readSelection}>Read selection aloud</button><label className="speech-rate">Speed<select value={speechRate} onChange={(event) => setSpeechRate(Number(event.target.value))}><option value="0.75">0.75×</option><option value="0.9">0.9×</option><option value="1">1×</option><option value="1.15">1.15×</option><option value="1.3">1.3×</option><option value="1.5">1.5×</option><option value="1.7">1.7×</option></select></label>{speechState !== "idle" && <><button type="button" onClick={toggleSpeech}>{speechState === "playing" ? "Pause" : "Resume"}</button><button type="button" onClick={stopSpeech}>Stop</button></>}</div></div>{speechText && <p className="reader-speech-status">Speaking selection locally in your browser at {speechRate}×.</p>}<div className="reader-summary reader-summary--document"><AiSummary paper={paper} pdf={summaryPdf} /></div><div className="reader-pages" ref={textPaneRef}>{pages.map((page, index) => <article className="reader-page" data-reader-page={index + 1} key={index}><p className="eyebrow">Page {index + 1}</p><div>{highlightedText(page, annotations.filter((item) => item.page === index + 1))}</div></article>)}</div></>}
