...
export default function ReadingRoom({ paper, annotations, onAddAnnotation, onUpdateAnnotation, onDeleteAnnotation, onEnsureSaved, onClose }: Props) {
  ...
  return <div className="reading-room" role="dialog" aria-modal="true" aria-labelledby="reading-room-title">
    ...
    <div className="reading-room__toolbar"><div>{readerState === "ready" && <><button className={mode === "text" ? "is-active" : ""} type="button" onClick={() => setMode("text")}>Selectable text</button><button className={mode === "pdf" ? "is-active" : ""} type="button" onClick={() => setMode("pdf")}>Original PDF</button></>}<button type="button" onClick={copyCitation}>Copy citation</button><button type="button" onClick={() => downloadText(`${paper.title.replace(/[^a-z0-9]+/gi, "-").toLowerCase()}.ris`, citationToRis(paper), "application/x-research-info-systems")}>Export RIS</button></div><div><label className="reader-upload">Upload PDF<input className="sr-only" type="file" accept="application/pdf,.pdf" onChange={(event) => { void uploadPdf(event.target.files?.[0]); event.target.value = ""; }} /></label>{documentSource === "user-uploaded PDF" && <button type="button" onClick={() => void forgetLocalPdf()}>Forget local PDF</button>}{paper.openAccessUrl && <a href={paper.openAccessUrl} target="_blank" rel="noreferrer">Open OA source ↗</a>}</div></div>
    ...
    <aside className="annotation-panel" aria-label="Notes and highlights"><AiSummary paper={paper} extractedText={documentSource === "user-uploaded PDF" ? pages.join("\n\n") : null} /><div className="annotation-compose"><p className="eyebrow">New annotation</p>{draftQuote ? <blockquote><span>Selected{draftPage ? ` · page ${draftPage}` : ""}</span>{draftQuote}</blockquote> : <p className="annotation-compose__hint">Capture a passage in Selectable text, or add a paper-level note without a highlight.</p>}<label>Note<textarea value={draftNote} onChange={(event) => setDraftNote(event.target.value)} rows={4} placeholder="Why does this passage matter?" /></label><button type="button" onClick={saveAnnotation}>Save annotation</button><p className="reader-action-status" aria-live="polite">{actionStatus}</p></div>
